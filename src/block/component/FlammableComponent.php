...
final class FlammableComponent implements BlockComponent {
	 ...
	 * @param bool $lavaFlammable Whether the block can be ignited by adjacent lava.
	 */
	public function __construct(int $catchChanceModifier = 5, int $destroyChanceModifier = 20, bool $lavaFlammable = false) {
		if($catchChanceModifier < 0){
			throw new \InvalidArgumentException("catchChanceModifier must be greater than or equal to 0");
		}
		if($destroyChanceModifier < 0){
			throw new \InvalidArgumentException("destroyChanceModifier must be greater than or equal to 0");
		}	
		$this->catchChanceModifier = $catchChanceModifier;
		$this->destroyChanceModifier = $destroyChanceModifier;
		$this->lavaFlammable = $lavaFlammable;
	}
	...
	public function getValue(): array {
		return [
			...
			"lava_flammable" => $this->lavaFlammable
		];
	}
}